...
class CourseController extends Controller
{
    ...
    public function view(string $slug)
    {
        ...
        if ($course->status != 1) {
            if (! Auth::user() || ! Auth::user()->isInstructorInCourse($course->id)) {
                abort(404);
            }
        }
        $isEnrolled = false;
        if (Auth::check()) {
            $enrolled = Auth::user()->isEnrolled($course->id);
            if ($enrolled) {
                $isEnrolled = $enrolled;
            }
        }

        return view('theme::course', compact('course', 'isEnrolled'));
    }
     ...
     */
    public function lectureView($slug, Content $content)
    {
        ...
        $isOpen = (bool) $content->is_preview;

        if ($content->course->paid && Auth::user()) {
            $isEnrolled = Auth::user()->isEnrolled($content->course->id);
            if ($content->course->paid && $isEnrolled) {
                $isOpen = true;
            }
        } elseif ($content->course->free) {
            if ($content->course->require_enroll && Auth::user()) {
                $isEnrolled = Auth::user()->isEnrolled($content->course->id);
                if ($isEnrolled) {
                    $isOpen = true;
                }
            } elseif ($content->course->require_login) {
                if (Auth::user()) {
                    $isOpen = true;
                }
            } else {
