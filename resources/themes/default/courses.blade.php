            ...
            <div class="container">
                ...
                <div class="row">
                                    ...
                                    @foreach(course_levels() as $key => $level)
                                        <label class="custom-control custom-checkbox">
                                            <input type="checkbox" class="custom-control-input" name="level[]"
                                                   value="{{$key}}" {{in_array($key, $old_level) ? 'checked="checked"' : ''}} >
                    ...
                    <div class="col-md-9">
                        ...
                        <div class="course-sorting-form-wrap form-inline mb-4">
                            ...
                            <div class="form-group mr-2">
                                ...
                                <select class="form-control" name="limit">
                                    @for($i = 10; $i<=100; $i = $i + 10)
                                        <option value="{{$i}}" {{selected($i, request('limit'))}}>{{$i}}</option>
                                    @endfor
                                </select>
                            </div>
                            ...
                            <div class="form-group">
                                <select class="form-control mr-2" name="sort">
                                    <option value="relevance" {{selected('relevance', request('sort'))}}>Most Relevant
                                    </option>
                                    <option value="most-reviewed" {{selected('most-reviewed', request('sort'))}}>Most
                                        Reviewed
                                    </option>
                                    <option value="highest-rated" {{selected('highest-rated', request('sort'))}}>Highest
                                        Rated
                                    </option>
                                    <option value="newest" {{selected('newest', request('sort'))}}>Newest</option>
                                    <option value="price-low-to-high" {{selected('price-low-to-high', request('sort'))}}>
                                        Lowest Price
                                    </option>
                                    <option value="price-high-to-low" {{selected('price-high-to-low', request('sort'))}}>
                                        Highest Price
                                    </option>
                                </select>
                            </div>
                            ...
                            <div class="form-group ml-auto">
                                <a href="{{route('courses')}}" class="btn btn-link"> <i class="las la-refresh"></i>
                                    Clear Filter</a>
                            </div>
                        </div>
                        ...
                        @if($courses->count())
                            <p class="text-muted mb-3"> Showing {{$courses->count()}} from {{$courses->total()}}
                                results </p>
